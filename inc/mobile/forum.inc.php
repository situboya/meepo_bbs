       ...
       $where = " AND 1 ";
        if($id){ $return = check_look($id);
         if(is_error($return)){ message($return['message'],$this->createMobileUrl('forum'),error); }
          ...
          $topics = pdo_fetchall($sql,$params); }else{ $sql = "SELECT * FROM ".tablename('meepo_bbs_topics')." WHERE uniacid = :uniacid $where ORDER BY last_reply_at DESC limit $start,10"; 
          	...
          	if(!empty($_W['isajax'])){ 
          		...
          		$data = ob_get_contents(); ob_clean();
          		 die($data);
          		  	...
          		  	include $this->template($tempalte.'/templates/forum/index');
          		  	 } 